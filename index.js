...
const textSprite = createTextSprite('Welcome to a Hobbit House', 40, '#ffffff');
...
const shape = new THREE.Mesh(sphereGeometry, material);
shape.position.set(4, 4, 10); 
shape.name = 'ToggleObject'; // Name the object for identification
scene.add(shape);
...
let isSphere = true; // State to track current geometry
...
window.addEventListener('click', (event) => {
    ...
    if (intersects.length > 0) {
        ...
        // Check if the clicked object is the toggle object
        if (clickedObject.name === 'ToggleObject') {
            console.log('Object clicked! Toggling shape.');

            // Toggle between sphere and cube
            clickedObject.geometry.dispose(); // Clean up old geometry
            if (isSphere) {
                clickedObject.geometry = boxGeometry; // Change to cube
            } else {
                clickedObject.geometry = sphereGeometry; // Change to sphere
            }
            isSphere = !isSphere; // Toggle state
        }
    }
});
